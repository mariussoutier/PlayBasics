...
object ActionComposition extends Controller {
  ...
  object Security {
    ...
    private def checkPermissions[A](request: TokenRequest[A], permissions: Seq[Permission], block: (CurrentUserRequest[A]) => Future[Result]): Future[Result] = {
    ...
    def hasPermissionPatternMatching(permissions: Permission*) = new ActionBuilder[CurrentUserRequest] {
      def invokeBlock[A](request: Request[A], block: (CurrentUserRequest[A]) => Future[Result]): Future[Result] =
        request match {
    ...
    def hasPermission(permissions: Permission*) = new ActionBuilder[CurrentUserRequest] {
      def invokeBlock[A](request: Request[A], block: (CurrentUserRequest[A]) => Future[Result]): Future[Result] = {
        val hasTokenHandler: TokenRequest[A] => Future[Result] = { tokenRequest =>
